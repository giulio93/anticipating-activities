...
def read_sequences(filename, recog_dir, obs_percentage,action_names,label2index):
    ...
    y_true = [label2index[w] for w in ground_truth]
    y_pred = [label2index[c] for c in recognized]

    plot_label_seq(y_true, 48, actions=classes,cmap=cmap, output_file="./images/"+filename.split('/')[5].split('.')[0]+'gt_'+'.png', legend=True)
    plot_label_seq(y_pred, 48, actions=classes,cmap=cmap, output_file="./images/"+filename.split('/')[5].split('.')[0]+'.png', legend=True)
    print('Ground truth labels:')
    Image(filename="./images/"+filename.split('/')[5].split('.')[0]+'gt_'+'.png')
    print('Predicted labels:')
    Image(filename="./images/"+filename.split('/')[5].split('.')[0]+'.png')
...
for filename in filelist:
    ...
    gt, recog = read_sequences(filename, args.ground_truth_path, obs_percentage,actions,label2index) # gt e recog hano la stessa lunghezza
    if(recog != 0):
        totalGT +=gt
        totalRecog += recog
