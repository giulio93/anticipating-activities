...
clactions_dict = read_mapping_dict(args.mapping_file)
nClasses = len(actions_dict)ass ModelCNN:
  actions_dict = read_mapping_dict(args.mapping_file)
nClasses = len(actions_dict)  
  actions_dict = read_mapping_dict(args.mapping_file)
nClasses = len(actions_dict)  def __init__(self, nRows, nCols):
        self.input_vid = tf.placeholder('float', [None, nRows, nCols, 1], name='input_vid')
        self.target = tf.placeholder('float', [None, nRows, 48, 1], name='target')
        self.nRows = nRows
        self.nCols = nCols
    ...
    def __build(self):
        ...
        W_fc2 = self.__weight_variable([1024, self.nRows*48], 'W_fc2')
        b_fc2 = self.__bias_variable([self.nRows*48], 'b_fc2')
        ...
        prediction_unscaled = tf.reshape(pred_flat, [-1, self.nRows, 48, 1])
        ## l2 normalization
