...
from utils.helper_functions import read_mapping_dict, encode_content, write_predictions, get_label_length_seq,draw_plot
...
##################
    ...
    if args.model == "rnn":
        ...
        for vid in list_of_videos:
            ...
            if args.input_type == "gt":
                features[vid] = np.load("./data" + '/features/' + vid + '.npy')
                content_input = features[vid]
                vid_len=content_input.shape[1]
                file_ptr = open("./data/groundTruth/"+vid+".txt", 'r') 
                annotation = file_ptr.read().split('\n')[:-1] 
                
            for obs_p in obs_percentages:
               
                if args.input_type == "gt":
                    observed_annotation = annotation[:int(obs_p*vid_len)]
                    #Observe the corresponding percentage
                    observed_content = content_input[:,:int(obs_p*vid_len)]
                    ...
                    input_x = [np.reshape(observed_content, [args.nRows, 64, 1])]
                ...
                recognition = np.concatenate((observed_annotation,recognition))
