...
class ModelRNN:
    ...
    def __init__(self, nClasses, rnn_size, max_seq_sz, num_layers):
        ...
        self.target = tf.placeholder('float', [None, nClasses])
    ...
    def train(self, sess, model_save_path, batch_gen, nEpochs, save_freq, batch_size):
        ...
        loss += tf.reduce_mean(tf.square( gt_labels - predicted_labels ))
    ...
    def predict(self, sess, model_save_path, p_seq, actions_dict, T):
        ...
        label_pred = actions_dict.keys()[actions_dict.values().index(np.argmax(result[:-2]))]
